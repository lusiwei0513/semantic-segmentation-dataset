...
class DiceLoss(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        one_hot = F.one_hot(target.clamp(min=0), num_classes=self.num_classes)
...
def iou_from_cm(
    ...
    By default mIoU averages only foreground classes (excludes background),
    which better reflects part-segmentation quality on this task.
    """
