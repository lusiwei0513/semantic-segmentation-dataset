...
def force_bn_eval(model: torch.nn.Module) -> None:
    for m in model.modules():
        if isinstance(m, torch.nn.modules.batchnorm._BatchNorm):
            m.eval()


def run_epoch(model, loader, criterion, optimizer, device, scaler, train: bool, num_classes: int, bn_eval: bool = False):
    model.train(train)
    if train and bn_eval:
        force_bn_eval(model)
    total_loss = 0.0
...
def main() -> None:
    ...
    image_size = cfg["data"]["image_size"]  # int S×S 或 [H, W]
    ...
    # DeepLab ASPP + batch_size=1: BN on 1x1 pooled maps raises
    ...
    if int(cfg["train"]["batch_size"]) <= 1:
        ...
        for m in model.modules():
            if isinstance(m, torch.nn.modules.batchnorm._BatchNorm):
                ...
                frozen += 1
        print(f"batch_size=1: freeze {frozen} BatchNorm layers (eval + no grad)")
    ...
    num_classes = int(cfg["model"]["num_classes"])
    bn_eval = int(cfg["train"]["batch_size"]) <= 1
    for epoch in range(start_epoch, total_epochs + 1):
        tr_loss, tr_iou, tr_miou = run_epoch(
            model, train_loader, criterion, optimizer, device, scaler, True, num_classes, bn_eval=bn_eval
        )
        va_loss, va_iou, va_miou = run_epoch(
            model, val_loader, criterion, optimizer, device, None, False, num_classes, bn_eval=False
        )
        scheduler.step()
