...
def save_prediction_visualization(
    ...
    title: str = "",
    valid_seg_tasks: Optional[torch.Tensor] = None,
) -> None:
    """Overlay only valid multi-label channels.

    Channel order: body, windshield, bogie, door.
    Front: bogie/door invalid — must not paint (otherwise looks like pink/tan noise).
    Side: all four valid.
    """
    ...
    colors = np.array(
        [
            [0.2, 0.6, 1.0],   # body — blue
            [1.0, 0.85, 0.2],  # windshield — yellow
            [0.2, 0.9, 0.4],   # bogie — green
            [1.0, 0.3, 0.3],   # door — red
        ],
        dtype=np.float32,
    )
    if valid_seg_tasks is None:
        valid = np.ones(probs.shape[0], dtype=np.float32)
    else:
        valid = valid_seg_tasks.detach().cpu().float().numpy().reshape(-1)
    overlay = img.copy()
    for i in range(min(probs.shape[0], len(colors))):
        if i >= len(valid) or float(valid[i]) < 0.5:
            continue
        m = probs[i] >= 0.5
